...
同样，k8s 组件鉴权是使用 `kubeconfig` 文件(内部包含 SSL pem 证书的 base64 内容和连接 apiserver 地址)，如果某个节点 kubeconfig 出问题或者过期，紧急处理的话可以先用正常的 kubectl 的 kubeconfig 拷贝到故障节点上的 kubelet cmdline 指定的 kubeconfig 路径（内部的 server url 根据需求看看是被否要修下）。
